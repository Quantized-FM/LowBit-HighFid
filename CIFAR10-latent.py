...
SAVE_PLOT_DIR = "latent_variance_plots"
...
# --------- LOADER ---------
def get_loader_cifar10(num_samples=10000):
    transform = transforms.Compose([transforms.ToTensor()])
    fullset = datasets.CIFAR10(root=DATA_ROOT, train=False, download=False, transform=transform)
...
def analyze_latent_shrinkage(latents, method, bits, dataset, save_dir=None, verbose=True):
    ...
    # Dead by mean absolute value
    mean_abs = np.abs(latents).mean(axis=0)
    dead_by_mean = np.sum(mean_abs < 1e-3)
    # Dead by unique values
    unique_counts = [len(np.unique(latents[:, i])) for i in range(latents.shape[1])]
    dead_by_unique = np.sum(np.array(unique_counts) < 3)

    if verbose:
        msg = (f"{dataset} | {method} @ {bits}-bit: mean_var={mean_var:.6f}, "
               f"std_var={std_var:.6f}, collapsed={collapsed_dims}/{len(var_per_dim)}, "
               f"dead_by_mean={dead_by_mean}, dead_by_unique={dead_by_unique}")
        write_log(msg)
    ...
    plt.close()
    return mean_var, std_var, collapsed_dims, dead_by_mean, dead_by_unique
...
test_images = get_loader_cifar10()
...
for method in quantization_methods:
    for bits in bit_widths:
        ...
        try:
            ...
            mean_var, std_var, collapsed_dims, dead_by_mean, dead_by_unique = analyze_latent_shrinkage(
            ...
            results.append({
                ...
                "latent_dead_by_mean": dead_by_mean,
                "latent_dead_by_unique": dead_by_unique,
            })
