...
SAVE_PLOT_DIR = "latent_variance_plots_celeba"
...
# --------- LOADER ---------
def get_loader_celeba(num_samples=10000):
    data_root = "/path/to/celeba/data"
    img_dir = Path(data_root) / "img_align_celeba_64"
    attr_path = Path(data_root) / "list_attr_celeba.txt"

    # Attributes file is for labels (optional; you may skip if not needed)
    transform = transforms.Compose([transforms.ToTensor()])

    # Parse filenames
    df_attr = pd.read_csv(attr_path, delim_whitespace=True, skiprows=1)
    filenames = df_attr.index.tolist()
    ...
    fullset = CelebADataset(img_dir, filenames, transform=transform)
    ...
    # batch is either a tensor [N, 3, 64, 64] or a list of tensors
...
test_images = get_loader_celeba()
args_filepath = BASE_CKPT_PATH + f"{DATASET}-args.json"
with open(args_filepath, 'r') as f:
    args_dict = json.load(f)

model_key = "imagenet"   # for CelebA your model is often called "imagenet"
device = torch.device('cuda:0')
# ... quantization/analysis loop ...
